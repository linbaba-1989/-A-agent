...
def render_topbar_status(target: Any, status: dict) -> None:
    ...
    target.markdown(f"<span style='font-size:12px'><b>{status['provider']}</b> {dot}　{display_status['market']}　"
                    f"行情：{display_status['quote_status']}　最后行情：{str(display_status['last_quote_time']).replace('T', ' ')}</span>",
                    unsafe_allow_html=True)


def render_topbar(status: dict) -> tuple[str, str, Any]:
    ...
    market_open = status.get("market_session") == OPEN
    ...
    market_status_slot = market.empty()
    render_topbar_status(market_status_slot, status)
    symbol = normalize_symbol(query)
    if symbol:
        st.session_state.selected_symbol = symbol
    return symbol, analysis_mode_value(analysis_mode), market_status_slot
